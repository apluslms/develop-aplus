...
- Most of the settings are defined in `config.yaml`
  - If there doesn't exist a `config.yaml` -file, the [`generate-submissions.sh`](/generate-submissions.sh) script will create one by copying the file [`config.example.yaml`](config.example.yaml).
...
- If the variable `global_duration_minutes` is defined, submissions are generated until the script stops when that many minutes have passed (or the script is stopped with CTRL+C).
  (I.e. the count variables are ignored.)
...
- In the directory, create solution files to be submitted.
  ...
  - For **programming exercises** or other exercises that one would submit a file, you should create a subdirectory for each "file selection box", where you place the submission files.
...
- Add the URIs to the `congif.yaml` file
  ...
  - Programming exercises and exercises requiring file submissions under the `submit` group
    - The exercise URI is provided as a variable name referring to a dict of the files.
      ...
      The value is the name of the subdirectory which contains the alternative submission files for the file.
      E.g.
        ```yaml
        uris:
